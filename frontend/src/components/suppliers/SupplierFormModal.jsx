...
/**
 * Add/Edit Supplier form. Pass `initialData` (a supplier object, with an
 * optional `purchaseCount`) to edit, or omit it to create. Same plain
 * useState + manual validation pattern as CustomerFormModal — no form
 * library introduced.
 *
 * When editing a supplier that already has purchases, Opening Due is
 * shown read-only: the backend rejects that edit outright (see
 * supplierController.updateSupplier) to protect historical balance
 * calculations, and the form reflects that same rule up front instead
 * of letting the person hit a server error.
 */
const SupplierFormModal = ({ open, onClose, onSubmit, initialData, isSubmitting }) => {
  const [form, setForm] = useState(emptyForm);
  const [error, setError] = useState('');

  const openingDueLocked = Boolean(initialData && (initialData.purchaseCount || 0) > 0);
  ...
  return (
    <Modal open={open} onClose={onClose} title={initialData ? 'Edit Supplier' : 'Add Supplier'}>
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {openingDueLocked && (
            <p className="mt-1 text-xs text-slate-400">
              Locked — this supplier already has purchase records.
            </p>
          )}
        </div>
        <div>
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          >
            {isSubmitting ? 'Adding...' : initialData ? 'Save Changes' : 'Add Supplier'}
          </button>
        </div>
      </form>
    </Modal>
